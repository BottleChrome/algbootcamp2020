...
bool bellmanFord(const vector<Edge>& graph, vector<int>& dist, int startV) {
    ...
    // Step 1. Loop 'n-1' times for updating distances.
    for (int i = 1; i < n; ++i) {
        for (int j = 0; j < graph.size(); ++j) {
            int sv = graph[j].s, ev = graph[j].e, cost = graph[j].w;
            // if 'sv' is visited and should be updated,
            if (dist[sv] != INF && dist[ev] > dist[sv] + cost) dist[ev] = dist[sv] + cost;    
        }
    }
    // Step 2. Loop once more to check 'negative cycle'.
    for (int j = 0; j < graph.size(); ++j) {
        int sv = graph[j].s, ev = graph[j].e, cost = graph[j].w;
        if (dist[sv] != INF && dist[ev] > dist[sv] + cost) return false;
    }
    return true;
}
int main() {
